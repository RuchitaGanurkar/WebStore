...
import com.webstore.exception.cart.CartHistoryValidationException;
import com.webstore.exception.cart.EmptyCartHistoryException;
...
class CartHistoryServiceImplementationTest {
    ...
    void createCartHistory_cartNotFound() {
        ...
        // Changed: Service throws CartHistoryValidationException, not CartNotFoundException
        assertThrows(CartHistoryValidationException.class, () -> service.createCartHistory(dto));
    }
    ...
    @Test
    void getCartHistoriesByCartId_empty() {
        when(cartHistoryRepository.findByCartIdOrderByCreatedAtDesc(1L))
                .thenReturn(Collections.emptyList());

        // Service throws EmptyCartHistoryException when list is empty
        assertThrows(EmptyCartHistoryException.class, () -> service.getCartHistoriesByCartId(1L));
    }
    ...
    @Test
    void getCartHistoriesByCreatedBy_empty() {
        when(cartHistoryRepository.findByCreatedBy("admin")).thenReturn(Collections.emptyList());

        // Service throws EmptyCartHistoryException when list is empty
        assertThrows(EmptyCartHistoryException.class, () -> service.getCartHistoriesByCreatedBy("admin"));
    }
    ...
    @Test
    void getCartHistoriesByUpdatedBy_empty() {
        when(cartHistoryRepository.findByUpdatedBy("admin")).thenReturn(Collections.emptyList());

        // Service throws EmptyCartHistoryException when list is empty
        assertThrows(EmptyCartHistoryException.class, () -> service.getCartHistoriesByUpdatedBy("admin"));
    }
    ...
    @Test
    void getCartHistoriesByDateRange_empty() {
        LocalDateTime start = LocalDateTime.now().minusDays(1);
        LocalDateTime end = LocalDateTime.now();

        when(cartHistoryRepository.findByDateRange(start, end)).thenReturn(Collections.emptyList());

        // Service throws EmptyCartHistoryException when list is empty
        assertThrows(EmptyCartHistoryException.class, () -> service.getCartHistoriesByDateRange(start, end));
    }
}